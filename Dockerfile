...
RUN apt-get update -y && \
    apt-get upgrade -y && \
    apt-get install -y kmod s3fs python3
...
RUN echo '#!/bin/bash' > /docker-entrypoint.sh && \
    echo "echo ${AWS_S3_ACCESS_KEY_ID}:${AWS_S3_SECRET_ACCESS_KEY} > /root/.s3fs-creds" >> /docker-entrypoint.sh && \
    echo "s3fs ${AWS_S3_BUCKET} /mnt/bucket -o _netdev,allow_other,nonempty,umask=000,passwd_file=/root/.s3fs-creds,use_cache=/tmp" >> /docker-entrypoint.sh && \
    echo "chmod 400 /root/.s3fs-creds" >> /docker-entrypoint.sh
